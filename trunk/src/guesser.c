...
/*          fprintf(stderr, "Guesser: %dh:%dm %s-%s\n",mapstat[i].hits, mapstat[i].miss,mapstat[i].map->lang,mapstat[i].map->charset);*/
     ...
     u = (u && (forte_lang || ((server_lang || meta_lang) && (0 == strcasecmp( DpsLanguageCanonicalName(lang), DPS_NULL2EMPTY(lang0)) ))) );
     u = (u && (forte_charset || ((server_charset || meta_charset) && (0 == strcasecmp( DpsCharsetCanonicalName(charset), DPS_NULL2EMPTY(charset0)) ))) );
